...
namespace nexus {
  ...
  SiPMNerea::SiPMNerea(): GeometryBase(),
		    visibility_(0)

  {
    /// Messenger
    msg_ = new G4GenericMessenger(this, "/Geometry/NextNew/", "Control commands of geometry NextNew.");
    msg_->DeclareProperty("SiPMNerea_vis", visibility_, "SiPMNerea Visibility");
  }
  ...
  G4ThreeVector SiPMNerea::GetDimensions() const
  {
    return dimensions_;
  }
  ...
  void SiPMNerea::Construct()
  {
    // PACKAGE ///////////////////////////////////////////////////////

    G4double sipm_x = 2.4 * mm;
    G4double sipm_y = 2.4 * mm;
    ...
    G4Box* sipm_solid = new G4Box("SiPMNerea", sipm_x/2., sipm_y/2., sipm_z/2);

    G4Material* epoxy = materials::Epoxy();
    epoxy->SetMaterialPropertiesTable(opticalprops::GlassEpoxy());

    G4LogicalVolume* sipm_logic =
      new G4LogicalVolume(sipm_solid, epoxy, "SiPMNerea");

    this->SetLogicalVolume(sipm_logic);

    // ACTIVE WINDOW /////////////////////////////////////////////////

    G4double active_side     = 2.4   * mm;
    ...
    new G4PVPlacement(0, G4ThreeVector(0, 0., -sipm_z/2. + active_depth/2.), active_logic,
    ...
    G4String sdname = "/SiPMNerea/SiPM";
